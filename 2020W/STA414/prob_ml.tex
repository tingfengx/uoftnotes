...
\end{equation*}
where we want to compute $P(D)$, with no observation\unsure{what is meant by no observation here?} for other variables. We have\note{$X_F$ is the observed set, $X_E$ is the un-observed set, and $X_R$ is everything that is remaining.}
\begin{equation*}
...
\begin{equation*}
    \tau(Y)=\sum_{z}\prod_{\phi \in \Phi} \phi(\mathrm{Scope}(\phi) \cap Z, \mathrm{Scope}(\phi) \cap Y)\quad \forall Y
\end{equation*}
...
\begin{equation*}
    P\left(X_{1: n}\right)=\frac{1}{Z} \prod_{i \in\{1,\dots,n\}} \phi\left(x_{i}\right) \prod_{(i, j) \in T} \phi_{i, j}\left(x_{i}, x_{j}\right)
\end{equation*}
and thus, if we want to compute $P(X_1)$ we can marginalize out all other variables
\begin{align*}
    P(X_1) = \sum_{x_2,\dots,x_n} P(X_{1:n}) 
    &= \sum_{x_2,\dots,x_n} \frac{1}{Z} \prod_i \phi\left(x_{i}\right) \prod_{(i, j) \in T} \phi_{i, j}\left(x_{i}, x_{j}\right) \\
    &\propto \sum_{x_2,\dots,x_n}  \prod_i \phi\left(x_{i}\right) \prod_{(i, j) \in T} \phi_{i, j}\left(x_{i}, x_{j}\right)
\end{align*}
