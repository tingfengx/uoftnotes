...
\begin{itemize}
    ...
    \item The goal is to add up to sum number $t = 1111111111 = \sum_i' b_{i'}$
\end{itemize}
...
\begin{itemize}
    ...
    \item After the two terms described above, we first note that they can't be on the same side in the partition, because if they do then one side would have sum at least $\sigma + t + 2\sigma - t = 3 \sigma$ which is way larger than $\sigma$, the total sum of all numbers. 
    ...
    \item Then, it suffices to find partitions that sums to $t$\footnote{$t$ here is some arbitray given value, so we can call the Subset Sum algo using this.} where we can use the Subset Sum algorithm. \qed
\end{itemize}


% \subsection{My Thorughts}
% \begin{itemize}
%     \item In doing reduction, try to show that some special case of the current problem could be solved by a known hard problem's solution in polynomial time. 
% \end{itemize}
