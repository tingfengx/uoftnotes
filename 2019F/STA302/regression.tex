...
\paragraph{\color{Thistle} \textit{t}-test using PMCC on Bivariate Normal \color{Black}}
...
Define \color{Thistle} Total Sample Variablility \color{Black}
\begin{equation*}
...
\paragraph{\textit{t}-test} Note that for SLR, this is equivalent to the F-test ourlined below. Consider the null $H_0: \beta_1 = 0$ against alternative $H_1: \beta_1 \neq 0$, then
\begin{equation*}
    T=\frac{\hat{\beta}_{1}-0}{\operatorname{se}\left(\hat{\beta}_{1}\right)} = \frac{\hat{\beta}_{1}- 0}{S / \sqrt{S X X}} \overset{H_0}{\sim} t_{(df = n-2)} 
\end{equation*}
This is just a specific application of the genral $t$-test that we mentioned earlier, not very interesting. 
...
\section{Selected Properties and Formulae}
