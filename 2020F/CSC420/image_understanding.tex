...
\paragraph{Definition (Blob)} A blob in a image is a patch of pixels that share some common property. e.g. they all have the same grayscale intensity values. 
...
\begin{align}
	...
	&I_0 = I * G_{k^0\sigma}
\end{align}
Then we compute their differences (element-wise). There should be $s$ difference maps, i.e. $I_1 - I_0, I_2 - I_1, ...$. Once we are done with the current scale, we will do it for the next one. (this is why we need the gaussian pyramid in the first place.) Finally, we find local maxima in scale, and do a bit of pruning of bad maxima.\footnote{Look over every $x$ and $y$, what is the highest response in the scale space? We will keep all points with response greater than a certain threshold. } We are done! 
